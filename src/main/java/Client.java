...
public class Client {
    ...
    public static void main(String[] args) throws UnknownHostException, IOException, LineUnavailableException {
        ...
        //Pergunta as acoes que o cliente deseja realizar
        while(option != 4){
            do {
            ...
            switch (option) {
                //Requisita uma lista de streams ao servidor
                case 1: {
                    ...
                    if (MessageManager.getMessageType(response) == MessageType.CONFIRMATION) {
                        ...
                        //Mostra a lista de streams retornada pelo servidor
                        if (parameters != null) {
                        ...
                        //Requisita um stream ao servidor, para ser escutado
                        ...
                        //Caso o stream exista, é realizada a conexão com o servidor de stream
                        if (MessageManager.getMessageType(response) == MessageType.CONFIRMATION) {
                    ...
                    //Requisita uma lista de músicas ao servidor
                    PrintStream saida = new PrintStream(client.getOutputStream());
                    ...
                    //Mostra a lista de músicas retornada pelo servidor
                    if (MessageManager.getMessageType(response) == MessageType.CONFIRMATION) {
                        String[] parameters = MessageManager.getMessageParameters(response);
                        ...
                        try {
                            //Requisita uma música ao servidor
                            saida = new PrintStream(client.getOutputStream());
                            ...
                            //Caso a música exista, a mesma é transferida do servidor e em seguida reproduzida no cliente
                            if(MessageManager.getMessageType(response) == MessageType.CONFIRMATION) {
                    ...
                    //Requisita a criação de um novo servidor de stream ao servidor
                    try {
                        ...
                        //É informado ao servidor todos os dados necessários para que outros clientes possam se conectar a esse stream
                        PrintStream saida = new PrintStream(client.getOutputStream());
                        ...
                        if (MessageManager.getMessageType(response) == MessageType.CONFIRMATION) {
                            ...
                            //Após o novo stream criado, o cliente entra em modo servidor, aguardando outros clientes se conectarem a ele
                            while (true) {
                                Socket listener = server.accept();
                                //Caso um novo cliente se conect ao strem, uma nova thread é disparada e o servidor volta a escutar
    ...
    //Bloco responsável pelo controle do áudio enviado para os ouvintes do stream
